  ...
  <aside className='flex h-full flex-col bg-[#090c12]'>
    ...
    <div className='flex flex-1 items-center justify-center p-4'>
      <div className='w-full rounded-lg border border-white/10 bg-black/90 p-2'>
        {activeVideoUrl ? (
          <video src={activeVideoUrl} controls className='aspect-video w-full rounded-md bg-black' />
        ) : (
          <div className='grid h-full place-items-center rounded-md border border-dashed border-white/10 p-6 text-center text-sm text-zinc-500'>
            Select or drop a video to preview
          </div>
        )}
      </div>
    </div>
    <div className='border-t border-white/5 px-6 py-3 text-xs text-zinc-400'>
